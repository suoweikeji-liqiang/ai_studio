...
4. 数据标签：提供跨数据集/字段的标签体系（如设备类型、季节、工况），支持批量打标、查询过滤与权限控制；内置示例标签：舒适、炎热、正常、故障、检修等场景/状态。

### 4.2 特征工程
1. 默认填充策略：全局与按字段/列名规则配置 ForwardFill/Zero/Mean/Median/Custom，可按列名/正则自动匹配策略并写入特征计划。（ref/M1.1 业务流程补全与交互优化方案.md:30-33）
2. 组合过滤：AND/OR 条件、阈值/区间/字符串匹配，支持可视化条件列表、条件模板与撤销；新增“高级筛选器”可构建多层嵌套表达式并保存复用。（ref/M1.1 业务流程补全与交互优化方案.md:32-33）
3. 公式表达式：
   - 内置函数 `lag`, `rolling`, `diff`, `zscore`, `poly`，生成新特征列；需要安全解析器避免任意执行。（ref/M1.1 业务流程补全与交互优化方案.md:33-38）
4. 计划保存与复用：特征页可保存/加载计划，探索页“一键加入特征计划”；提供预览与成本评分。
5. 自动超参调节：训练前可触发 Auto Tuning，按算法类型自动搜索超参空间并回写最优配置。（ref/M1.1 业务流程补全与交互优化方案.md:37-39, ref/数据探索.tsx:20-195）
...
3. 相似度分析：基于天气/负荷等关键特征，计算与今天或明天最相似的历史日期，输出参考指标及策略建议。
4. 智能探索：以分类卡片形式沉淀常见问题（能效、异常、设备健康等），并以工况分类（如舒适/炎热/检修/故障）呈现，支持一键跳转至对应分析视图。
...
4. 多算法训练：同一数据集可挂载多个训练方法（模板/脚本/导入），支持并行实验及结果对比。
5. 试运行：已训练模型需提供试运行沙箱，允许上传样例/实时数据验证输出与后处理逻辑。
...
- 训练资源：暂未指定，优先考虑以 Airflow 调度 + MLflow 跟踪的组合；自定义镜像与脚本运行暂不需要审批流程，可直接由工程团队配置。Auto Tuning 默认运行在 CPU 资源池，暂不调度 GPU。当前部署环境为单台 16 核 CPU / 96GB 内存 + RTX 3090，需在该资源范围内排期。
