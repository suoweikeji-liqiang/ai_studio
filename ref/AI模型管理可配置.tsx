...
type ModelConfig = {
  ...
  // Update: Image Config with Config Injection
  imageConfig?: { 
      imageUrl: string; 
      command: string; 
      envs: {key:string, value:string}[];
      runConfigMountPath?: string; // e.g., /app/config/config.yaml
      runConfigContent?: string;   // Content of the config file
  };

  // Update: Script Config with Config File Support
  scriptConfig?: { 
      sourceType: 'online_edit' | 'git_repo'; 
      code?: string; 
      gitUrl?: string; 
      gitBranch?: string;
      runConfigFile?: string;     // Path to config file in Git repo (e.g., configs/model_a.yaml)
      runConfigContent?: string;  // Content of config file (for online edit)
      startupCommand?: string;    // e.g., python train.py --config ${config_file}
  };
...
function ModelListView({ models, onSelect, onCreate, onUpdateTags }: any) {
  ...
  const allTags = useMemo(() => {
      ...
      models.forEach((m: any) => (m.tags || []).forEach((t: any) => tags.add(t)));
...
function ModelWizard({ initialData, isEdit, existingTags = [], onBack, onSave }: any) {
  ...
  const [formData, setFormData] = useState<Partial<ModelConfig>>({
    ...
    scriptConfig: { 
        sourceType: 'online_edit', 
        code: '# 训练脚本\nimport pandas as pd...',
        runConfigContent: '# Runtime Configuration\nmodel_type: xgboost\nparams:\n  learning_rate: 0.01' 
    },
  ...
  // New State for Script Editor Tabs
  const [activeScriptTab, setActiveScriptTab] = useState<'code' | 'config'>('code');
  ...
  const handleRunSandbox = () => { 
      ...
      setSandboxLogs(['> Initializing environment...', '> Pip installing dependencies...', '> Loading runtime config...', '> Running script...']);
      setTimeout(() => { 
          setSandboxLogs(prev => [...prev, '[INFO] Config loaded: ' + (formData.scriptConfig?.runConfigFile || 'inline config'), '[INFO] Training started', '[SUCCESS] Model saved to /output/model.pkl']); 
          ...
          <div className="flex-1">
            ...
            {step === 3 && (
              <div className="space-y-4 animate-in slide-in-from-right-4 fade-in h-full flex flex-col">
                ...
                {/* 3B. Script Mode (UPDATED for Runtime Config) */}
                    ...
                    <div className="flex gap-6 flex-1 min-h-[400px]">
                        <div className="flex-1 flex flex-col gap-4">
                            ...
                            <div className="bg-slate-100 p-2 rounded-lg flex items-center gap-2 border border-slate-200">
                                ...
                                >
                                    <option value="online_edit">在线编辑 (Online)</option>
                                    <option value="git_repo">Git 仓库 (Repository)</option>
                                </select>
                                {formData.scriptConfig?.sourceType === 'git_repo' && (
                                    <>
                                        ...
                                        <input 
                                            ...
                                            placeholder="Branch" 
                                            value={formData.scriptConfig?.gitBranch || ''}
                                            onChange={e => setFormData({...formData, scriptConfig: {...formData.scriptConfig!, gitBranch: e.target.value}})}
                                        />
                                    </>
                                )}
                            </div>
                            ...
                            {/* Git Config Input */}
                            {formData.scriptConfig?.sourceType === 'git_repo' && (
                                <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm animate-in slide-in-from-top-2">
                                    <h4 className="text-xs font-bold text-slate-500 uppercase mb-3 flex items-center gap-2">
                                        <FileCog className="w-4 h-4 text-indigo-500"/> 运行配置文件 (Runtime Config)
                                    </h4>
                                    <div className="flex gap-4">
                                        <div className="flex-1">
                                            <label className="block text-[10px] text-slate-400 mb-1">配置文件路径 (Repo Path)</label>
                                            <input 
                                                type="text" 
                                                className="w-full border border-slate-300 rounded px-3 py-2 text-sm font-mono"
                                                placeholder="e.g. configs/production_v1.yaml"
                                                value={formData.scriptConfig?.runConfigFile || ''}
                                                onChange={e => setFormData({...formData, scriptConfig: {...formData.scriptConfig!, runConfigFile: e.target.value}})}
                                            />
                                        </div>
                                        <div className="flex-1">
                                            <label className="block text-[10px] text-slate-400 mb-1">启动命令 (Startup Command)</label>
                                            <input 
                                                type="text" 
                                                className="w-full border border-slate-300 rounded px-3 py-2 text-sm font-mono"
                                                placeholder="python train.py --config ${config_file}"
                                                value={formData.scriptConfig?.startupCommand || ''}
                                                onChange={e => setFormData({...formData, scriptConfig: {...formData.scriptConfig!, startupCommand: e.target.value}})}
                                            />
                                        </div>
                                    </div>
                                    <p className="text-[10px] text-slate-400 mt-2">
                                        提示：通过指定不同的配置文件，可以使用同一套 Git 代码库构建多个不同行为的模型。
                                    </p>
                                </div>
                            )}

                            {/* Online Editor Area */}
                            {formData.scriptConfig?.sourceType === 'online_edit' && (
                                <div className="flex-1 flex flex-col gap-0 border border-slate-300 rounded-xl overflow-hidden shadow-inner">
                                    {/* Editor Tabs */}
                                    <div className="flex bg-[#252526] border-b border-[#3e3e42]">
                                        <button 
                                            onClick={() => setActiveScriptTab('code')}
                                            className={`px-4 py-2 text-xs font-medium border-r border-[#3e3e42] flex items-center gap-2 ${activeScriptTab === 'code' ? 'bg-[#1e1e1e] text-white' : 'text-[#969696] hover:bg-[#2d2d2d]'}`}
                                        >
                                            <Code className="w-3 h-3"/> main.py
                                        </button>
                                        <button 
                                            onClick={() => setActiveScriptTab('config')}
                                            className={`px-4 py-2 text-xs font-medium border-r border-[#3e3e42] flex items-center gap-2 ${activeScriptTab === 'config' ? 'bg-[#1e1e1e] text-white' : 'text-[#969696] hover:bg-[#2d2d2d]'}`}
                                        >
                                            <Settings className="w-3 h-3"/> config.yaml
                                        </button>
                                    </div>
                                    ...
                                    <div className="flex-1 bg-[#1e1e1e] relative">
                                        <textarea 
                                            className="w-full h-full bg-transparent text-[#d4d4d4] p-4 font-mono text-sm outline-none resize-none"
                                            value={activeScriptTab === 'code' ? (formData.scriptConfig?.code || '') : (formData.scriptConfig?.runConfigContent || '')}
                                            onChange={e => {
                                                const val = e.target.value;
                                                if (activeScriptTab === 'code') {
                                                    setFormData({...formData, scriptConfig: {...formData.scriptConfig!, code: val}});
                                                } else {
                                                    setFormData({...formData, scriptConfig: {...formData.scriptConfig!, runConfigContent: val}});
                                                }
                                            }}
                                            spellCheck={false}
                                            placeholder={activeScriptTab === 'code' ? "# Enter your training code here" : "# Enter runtime configuration here\nepoch: 100\nlr: 0.01"}
                                        />
                                        
                                        {/* Sandbox Overlay */}
                                        {sandboxStatus !== 'idle' && (
                                            <div className="absolute bottom-0 w-full h-32 bg-black/90 text-slate-300 p-3 font-mono text-xs overflow-y-auto border-t border-slate-700 z-10">
                            ...
                            <div className="flex-1 p-3 bg-slate-50 overflow-y-auto">
                                <div className="text-xs text-slate-500 mb-2">
                                    {activeScriptTab === 'code' 
                                        ? "Ask AI to generate training code or data processing logic." 
                                        : "Ask AI to suggest hyperparameters or configuration structures."}
                                </div>
                                {aiSuggestion && (
                            ...
                            <div className="p-3 border-t bg-white">
                                <div className="relative">
                                    <input 
                                        ...
                                        placeholder={activeScriptTab === 'code' ? "Generate code..." : "Generate config..."}
                                        value={aiPrompt}
                ...
                {(formData.buildMode === 'custom_image' || formData.buildMode === 'import') && (
                    <div className="space-y-6 max-w-xl">
                        ...
                        {/* Configuration Injection for Custom Images */}
                        {formData.buildMode === 'custom_image' && (
                            <div className="border-t border-slate-100 pt-4 mt-2">
                                <h4 className="text-sm font-bold text-slate-800 mb-3 flex items-center gap-2">
                                    <FileCog className="w-4 h-4 text-slate-500"/> 配置文件挂载 (Config Injection)
                                </h4>
                                <div className="space-y-3">
                                    <div>
                                        <label className="block text-xs font-medium text-slate-500 mb-1">容器内挂载路径 (Mount Path)</label>
                                        <input 
                                            type="text" 
                                            className="w-full border border-slate-300 rounded px-3 py-2 text-sm font-mono" 
                                            placeholder="/app/config/config.yaml"
                                            value={formData.imageConfig?.runConfigMountPath || ''}
                                            onChange={e => setFormData({...formData, imageConfig: {...formData.imageConfig!, runConfigMountPath: e.target.value}})}
                                        />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-medium text-slate-500 mb-1">配置文件内容 (Content)</label>
                                        <textarea 
                                            ...
                                            value={formData.imageConfig?.runConfigContent || ''}
                                            onChange={e => setFormData({...formData, imageConfig: {...formData.imageConfig!, runConfigContent: e.target.value}})}
                                        />
                                    </div>
                                </div>
                            </div>
...
// --- 4.3 Model Detail View (Restored) ---
